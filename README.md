...
Проєкт автоматично розгорнуто у хмарі (CD):
- **Frontend (Vercel):** (https://kpi-9sf6.vercel.app/)
- **Backend (Render):**  (https://lms-backend-9rq2.onrender.com)
...
1. Клонування репозиторію:
   ...
   ```bash
   git clone (https://github.com/zhuravlovO/KPI)
   ```
...
Переконайтеся, що у папці `server` існує файл `.env` із посиланням на базу даних:
...
npm run dev
```
Інтерфейс доступний за адресою: `http://localhost:5173`
